---
title: "The competitivePRS user's guide"
author: "Javier Costas"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The competitivePRS user's guide}
...
```{r eval = FALSE}
library(devtools)
install_github("costasjavier/competitivePRS")
```
...
Alternatively, the list may be read from a file:

```{r}
my.gene.list <- read.table("~/competitivePRS/data/ex_genelist.txt", header = F)
head(my.gene.list)
```

The second argument is a string with the full `path.to.plink.files`, where the three files of the **PLINK** binary
format are present. These files store the genotype data of the target sample file, the user's data. The third argument is a string indicating the name of the **PLINK** binary file, without extension. The four argument is an array of two numbers, indicating the length in kb of
...
```{r}
my.bim <- read.table("~/competitivePRS/data/exPLINK.bim")
head(my.bim)
my_SNP_list <- Map_genestoSNPs(geneset = my.gene.list, path.to.plink.files = "~/competitivePRS/data/", bfile = "exPLINK", extra.kb = c(35,10), output = "my_output_1")
head(my_SNP_list)
```
...
In this example, first I create the  `path.to.plink.files` argument, needed to read the **PLINK** files. Alternatively, the user may write the path directly.
...
```{r}
ex_summGWAS <- read.table("data/conc_GWAS", header = T)
head(ex_summGWAS)
```
...
```{r}
Covariates <- read.table("data/ex_Covariates", header = T)
head(Covariates)
```
